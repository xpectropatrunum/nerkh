        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center px-3">
                    ...
                    <a class="btn btn-outline-primary btn-sm mx-3" href="{{ route('admin.currencies.create') }}"><i class="fa fa-plus"></i> {{ __('admin.add_new') }}</a>
                </div>
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.currencies.index') }}" type="post" autocomplete="off">
                        @csrf
                        ...
                        <table class="table table-striped table-bordered mb-0 text-nowrap">
                            ...
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->slug }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->priority ?: 1000 }}</td>
                                    <td>{{ (new Shamsi)->jdate($item->created_at) }}</td>
                                    <td class="project-actions">
                                        <a class="btn btn-info btn-sm" href="{{ route('admin.currencies.edit',$item->id) }}">
                                            <i class="fas fa-pencil-alt"></i>
                                            Edit 
                                        </a>

                                        <form action="{{ route('admin.currencies.destroy',$item->id) }}" class="d-inline-block" method="POST">
                                            @csrf
                                            @method('DELETE')
