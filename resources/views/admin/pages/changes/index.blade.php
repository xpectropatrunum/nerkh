...
@section('title', 'currency diffs')

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">currency diffs</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb @if (app()->getLocale() == 'fa') float-sm-left @else float-sm-right @endif">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('admin.dashboard') }}</a></li>
                <li class="breadcrumb-item active">currency diffs</li>
            </ol>
        </div>
    </div>
@endsection
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center px-3">
                    <h3 class="card-title">currency diffs</h3>
                    <a class="btn btn-outline-primary btn-sm mx-3" href="{{ route('admin.changes.create') }}"><i
                            class="fa fa-plus"></i> {{ __('admin.add_new') }}</a>
                </div>
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.changes.index') }}" type="post" autocomplete="off">
                        @csrf
                                    ...
                                    <tr>
                                        ...
                                        <td class="project-actions">
                                            <a class="btn btn-info btn-sm"
                                                href="{{ route('admin.changes.edit', $item->id) }}">
                                                <i class="fas fa-pencil-alt"></i>
                                                Edit
                                            </a>

                                            <form action="{{ route('admin.changes.destroy', $item->id) }}"
