...
@section('title', 'currency prices')

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">currency prices</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb @if(app()->getLocale() == 'fa') float-sm-left @else float-sm-right @endif">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('admin.dashboard') }}</a></li>
                <li class="breadcrumb-item active">currency prices</li>
            </ol>
        </div>
    </div>
@endsection
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center px-3">
                    <h3 class="card-title">currency prices</h3>
                    <a class="btn btn-outline-primary btn-sm mx-3" href="{{ route('admin.prices.create') }}"><i class="fa fa-plus"></i> {{ __('admin.add_new') }}</a>
                </div>
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.prices.index') }}" type="post" autocomplete="off">
                        @csrf
                            ...
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td class="project-actions">
                                        <a class="btn btn-info btn-sm" href="{{ route('admin.prices.edit',$item->id) }}">
                                            <i class="fas fa-pencil-alt"></i>
                                            Edit 
                                        </a>

                                        <form action="{{ route('admin.prices.destroy',$item->id) }}" class="d-inline-block" method="POST">
                                            @csrf
                                            @method('DELETE')
